...
# pylint: disable=R0912, R0915, E1101, E1120, E1102, C0103, W0702, R0914, C0116, C0115
...
def main(cfg: DictConfig):
    ...
    cfg.problem.model.test_iterations = list(
        range(
            cfg.problem.model.test_iterations["low"],
            cfg.problem.model.test_iterations["high"] + 1,
        )
    )
    assert 0 <= cfg.problem.hyp.alpha <= 1, (
        "Weighting for loss (alpha) not in [0, 1], exiting."
    )
    ...
    #        Train
    log.info(
        f"==> Starting training for {max(cfg.problem.hyp.epochs - start_epoch, 0)} epochs..."
    )
    ...
    for epoch in range(start_epoch, cfg.problem.hyp.epochs):
        loss, acc = dt.train(
            net, loaders, cfg.problem.hyp.train_mode, train_setup, device
        )
        val_acc = dt.test(
            net,
            [loaders["val"]],
        ...
        for i in range(len(optimizer.param_groups)):
            writer.add_scalar(
                f"Learning_rate/group{i}", optimizer.param_groups[i]["lr"], epoch
            )

        # evaluate the model periodically and at the final epoch
        if (
            epoch + 1
        ) % cfg.problem.hyp.val_period == 0 or epoch + 1 == cfg.problem.hyp.epochs:
